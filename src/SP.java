...
public class SP implements Constants {
    ...
    SP(String file, String pubExp, String privExp, String mod) throws Exception {
        pTag = (byte) SPTAG;
        version = 3;
        length = 5; // always 5
        ...
        byte[] pub = Utilities.hex2Byte(pubExp);
        ...
        //signature = new MPI(RSA.RSAOperation(Utilities.getHexString(getHash(file)), privExp, mod));
        //EMSA_PKCS_Encoding(file,60);
        // 19 -- packet header, 2 + signature.len() -- MPI
        signature = new MPI(RSA.RSAOperation(Utilities.getHexString(EMSA_PKCS_Encoding(file, 50)), privExp, mod));
    ...
    public byte[] getHash(String file) throws Exception {
        ...
        while ((read = inputFile.read(dataBytes, offset, dataBytes.length - offset)) != -1) {
    ...
    public byte[] EMSA_PKCS_Encoding(String file, int emLen) throws Exception {
        ...
        for (int i = 0; i < PS.length; i++)
            PS[i] = (byte) 0xff;
        byte[] encodedMessage = new byte[3 + PS.length + T.length];
        encodedMessage[0] = (byte) 0x00;
        encodedMessage[1] = (byte) 0x01;
        for (int i = 0; i < PS.length; i++)
            encodedMessage[i + 2] = PS[i];
        encodedMessage[PS.length + 2] = 0x00;
        for (int i = 0; i < T.length; i++)
            encodedMessage[i + PS.length + 3] = T[i];
